...
class TestDataGenerator:
    ...
    def generate_test_data(api: Any) -> Dict[str, Any]:
            ...
            for param in api.parameters:
                ...
                if param_in == 'path':
                    ...
                    if isinstance(body_data, dict):
                        test_data['body'] = body_data if body_data else None
                    else:
                        test_data['body'] = None
        ...
        # 移除空值
        if not test_data['path_params']:
            test_data['path_params'] = None
        if not test_data['query_params']:
            test_data['query_params'] = None
        if not test_data['headers']:
            test_data['headers'] = None
        if not test_data['form_data']:
            test_data['form_data'] = None
        # body 已经被正确设置为 None 或 Dict
