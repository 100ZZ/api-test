...
class OpenAPIParser:
    ...
    def _resolve_ref(self, schema: Dict[str, Any], visited: set = None) -> Dict[str, Any]:
        ...
        Args:
            schema: 可能包含 $ref 的 schema
            visited: 已访问的引用集合，用于避免循环引用
        ...
        # 提取引用的 schema 名称
        # 支持格式: #/components/schemas/SchemaName 或 #/definitions/SchemaName
        if ref.startswith('#/'):
            parts = ref.split('/')
            if len(parts) >= 3:
                # parts: ['#', 'components', 'schemas', 'SchemaName'] 或 ['#', 'definitions', 'SchemaName']
                schema_name = parts[-1]
                ...
                if schema_name in self.schemas:
                    ...
                    # 找不到引用的 schema 定义
                    print(f"⚠️ 警告: 无法在 schemas 中找到引用: {ref}")
                    print(f"   可用的 schemas: {', '.join(list(self.schemas.keys())[:10])}...")
        
        # 如果找不到引用，返回一个默认的 object 类型
        print(f"⚠️ 警告: 无法解析引用: {ref}")
    ...
    def _extract_file_params_from_query(self, parameters: List[Dict[str, Any]], request_body: Dict[str, Any]) -> tuple:
        """
        智能识别文件上传参数：
        将 query 参数中明显是文件上传的参数移到 formData
        """
